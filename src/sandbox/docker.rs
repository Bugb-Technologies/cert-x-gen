...
impl DockerSandbox {
    ...
    pub async fn build_image(&self, dockerfile_path: Option<&Path>) -> Result<()> {
        ...
        let dockerfile = dockerfile_path.map(|p| p.to_path_buf()).unwrap_or_else(|| {
                    ...
                    if content.contains("credsStore") || content.contains("credStore") {
                        ...
                        if std::fs::copy(config_path, &backup_path).is_ok() {
                            ...
                            let modified = content
                                .lines()
                                .filter(|line| {
                                    !line.contains("credsStore") && !line.contains("credStore")
                                })
                                .collect::<Vec<_>>()
                                .join("\n");
        ...
        if !output.status.success() {
                ...
                || stderr.contains("error getting credentials")
            {
                ...
                return Err(Error::command(format!("Docker build failed: {}", stderr)));
            }
        }
    ...
    fn generate_dockerfile_content(&self) -> String {
        ...
        if self.config.languages.contains(&"node".to_string()) {
            ...
            content
                .push_str("RUN curl -fsSL https://deb.nodesource.com/setup_20.x | bash - && \\\n");
