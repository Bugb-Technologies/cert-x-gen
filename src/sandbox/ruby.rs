...
pub async fn init_environment(sandbox: &Sandbox) -> Result<()> {
    ...
    if let Ok(output) = ruby_version {
        ...
        if let Some(major_minor) = version.split('.').take(2).collect::<Vec<_>>().join(".").parse::<f32>().ok() {
            if major_minor < 3.0 {
                tracing::warn!("Ruby version {} is outdated. Some gems require Ruby 3.0+", version);
                tracing::warn!("Consider upgrading Ruby: brew install ruby (macOS) or rbenv install 3.2.0");
                tracing::warn!("Continuing with available Ruby version. Some gems may fail to install.");
            }
        }
    }

    let gem_check = Command::new("gem")
        .arg("--version")
        .output();
    ...
    tracing::info!("Installing {} Ruby gems (this may take a few minutes)...", gem_refs.len());
...
pub async fn install_gems(sandbox: &Sandbox, gems: &[&str]) -> Result<()> {
    ...
    for gem in gems {
        let output = Command::new("gem")
            .args(&["install", gem, "--install-dir", gem_home.to_str().unwrap(), "--no-document"])
            .output();
        ...
        match output {
            ...
            Ok(out) => {
                ...
                if combined_msg.contains("requires Ruby version") || combined_msg.contains("Ruby version") {
                    tracing::warn!("Skipping gem {} (requires newer Ruby version - current Ruby may be too old)", gem);
                } else if combined_msg.contains("ERROR") && combined_msg.contains("Error installing") {
                    // Extract the actual error reason
                    let error_lines: Vec<&str> = combined_msg.lines().filter(|l| l.contains("ERROR") || l.trim().starts_with("Error")).collect();
                    ...
                    let short_error = combined_msg.lines().filter(|l| !l.trim().is_empty() && !l.contains("Building")).take(2).collect::<Vec<_>>().join("; ");
                    tracing::warn!("Failed to install gem {}: {}", gem, if short_error.is_empty() { "Unknown error" } else { &short_error });
                }
            }
            Err(e) => {
                failed += 1;
    ...
    tracing::info!("Ruby gems installation complete: {} successful, {} failed/skipped", successful, failed);
    
    // Don't fail if at least some packages installed
    if successful > 0 {
        Ok(())
    } else if failed == gems.len() {
        tracing::warn!("All Ruby gems failed to install. Ruby sandbox may not be fully functional.");
