...
pub async fn init_environment(sandbox: &Sandbox) -> Result<()> {
    ...
    let cpanm_check = Command::new("cpanm")
        .arg("--version")
        .output();
    ...
    if cpanm_check.is_err() {
        ...
        if curl_check.is_err() {
            ...
            tracing::warn!("Please install cpanm manually: curl -L https://cpanmin.us | perl - App::cpanminus");
        ...
        let download_result = Command::new("curl")
            .args(&["-L", "https://cpanmin.us", "-o", sandbox_cpanm.to_str().unwrap()])
            .output();
        ...
        match install_result {
            ...
            Ok(_) => {
                ...
                tracing::info!("System-wide cpanm installation failed, will use sandbox-local version");
            }
            Err(e) => {
                tracing::info!("System-wide cpanm installation failed ({}), will use sandbox-local version", e);
            }
        }
    }
    ...
    tracing::info!("Installing {} Perl modules (this may take a few minutes)...", module_refs.len());
...
pub async fn install_modules(sandbox: &Sandbox, modules: &[&str]) -> Result<()> {
    ...
    if cpanm_check.is_err() {
        ...
        if sandbox_cpanm.exists() {
            ...
            tracing::warn!("Please install cpanm: curl -L https://cpanmin.us | perl - App::cpanminus");
        ...
        match output {
            ...
            Ok(out) => {
                ...
                if error_msg.contains("No such file or directory") {
                    tracing::warn!("Skipping module {} (cpanm not properly installed or module not found)", module);
                    tracing::debug!("Error details: {}", error_msg.lines().next().unwrap_or("Unknown error"));
                } else if error_msg.contains("Could not resolve") {
                    tracing::warn!("Skipping module {} (module name may be incorrect or not available)", module);
                } else {
                    tracing::warn!("Failed to install module {}: {}", module, error_msg.lines().take(2).collect::<Vec<_>>().join("; "));
                }
            }
            Err(e) => {
                failed += 1;
    ...
    tracing::info!("Perl modules installation complete: {} successful, {} failed/skipped", successful, failed);
    
    // Don't fail if at least some modules installed, or if we had failures but it's expected
    if successful > 0 {
        Ok(())
    } else if failed == modules.len() {
        tracing::warn!("All Perl modules failed to install. Perl sandbox may not be fully functional.");
        tracing::info!("You can install modules manually later using: cpanm -L {} <module>", local_lib.display());
