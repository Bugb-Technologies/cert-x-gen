    ...
    pub async fn apply_to_sandbox(&self, sandbox: &mut Sandbox) -> Result<()> {
        ...
        *sandbox = Sandbox::with_config(self.config.clone());
        ...
        if !self.python_packages.is_empty() {
            tracing::info!("Installing {} Python packages...", self.python_packages.len());
        ...
        if !self.javascript_packages.is_empty() {
            tracing::info!("Installing {} JavaScript packages...", self.javascript_packages.len());
            let packages: Vec<&str> = self.javascript_packages.iter().map(|s| s.as_str()).collect();
