...
pub async fn install_packages(sandbox: &Sandbox, packages: &[&str]) -> Result<()> {
    ...
    let filtered_packages: Vec<&str> = packages
        .iter()
    ...
    tracing::info!(
        "npm packages installation complete: {} successful, {} failed/skipped",
        successful,
        failed
    );
    ...
    if successful > 0 || (failed == 0 && successful == 0) {
        ...
        tracing::warn!(
            "All npm packages failed to install. JavaScript sandbox may not be fully functional."
        );
