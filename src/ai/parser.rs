...
impl ResponseParser {
    ...
    pub fn parse(&self, response: &str, language: TemplateLanguage) -> Result<String> {
        tracing::debug!("Parsing response ({} chars) for {:?}", response.len(), language);
    ...
    fn extract_python_code(&self, text: &str) -> String {
        ...
        for i in (start..lines.len()).rev() {
            ...
            let is_prose = prose_indicators.iter().any(|&indicator| trimmed.starts_with(indicator));
    ...
    fn extract_javascript_code(&self, text: &str) -> String {
        ...
        for (i, line) in lines[start..].iter().enumerate() {
            ...
            if trimmed.is_empty() {
                ...
                if prose_starters.iter().any(|&starter| trimmed.starts_with(starter)) {
                    end = start + i;
                    break;
                }
            }
    ...
    fn validate_python_structure(&self, code: &str) -> Result<()> {
        ...
        if !code.contains("def ") && !code.contains("class ") {
            ...
            tracing::error!("Code preview (first 500 chars): {}", &code.chars().take(500).collect::<String>());
...
mod tests {
    ...
    fn test_extract_python_code() {
        ...
        let input = "Here's a Python template:\n\n#!/usr/bin/env python3\nimport json\nprint('test')";
