...
impl JavaEngine {
    ...
    async fn execute_java_template(
        ...
        let class_name = template_path.file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("Template");
        let class_file = self.cache_dir.join(format!("{}-{}.class", class_name, cache_key));
    ...
    async fn compile_template(&self, source_path: &Path, _class_file: &Path) -> Result<()> {
        if !check_tool_available("javac").await {
            return Err(Error::Execution("Java compiler (javac) not found".to_string()));
        }
        ...
        if !output.status.success() {
            ...
            return Err(Error::Execution(format!("Java compilation failed: {}", stderr)));
        }
...
impl Template for JavaTemplate {
    async fn execute(&self, target: &Target, context: &Context) -> Result<Vec<Finding>> {
        self.engine.execute_java_template(&self.path, target, context).await
    }
