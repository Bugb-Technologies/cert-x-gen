...
| `@oracles` | Comma-separated | `asan, signal, exit` | How this template decides something is wrong. Vocabulary: `asan` `ubsan` `msan` `tsan` `signal` `exit` `assert` `timeout` `diff` `property` `detector`. Under `--require-instrumentation`, a template whose only oracles are sanitizers the build does not carry is recorded as `skipped / oracle-unavailable(...)` rather than run |
