...
- **`--scope-file` failed open.** It is how an operator tells `cxg pentest` what a scan is
  ...
  document that is not a mapping of settings, an empty file, or PyYAML being absent all
  raise `ScopeFileError`, and `run_pentest` stops with **exit 4** (a mis-specified run, not
