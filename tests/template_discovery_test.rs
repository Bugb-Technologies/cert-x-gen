...
use cert_x_gen::template::{TemplateManager, TemplateSource, PathResolver};
use std::fs;
...
async fn test_path_resolver_returns_valid_paths() {
    ...
    assert_eq!(local_dir, PathBuf::from("./templates"), "Local path should be ./templates");
    
    // Paths should be different
    assert_ne!(system_dir, user_dir, "System and user paths should differ");
    assert_ne!(system_dir, local_dir, "System and local paths should differ");
}
...
async fn test_multi_source_template_discovery() {
    ...
    create_test_template(temp_system.path(), "test-template-system.yaml", "System Template");
    create_test_template(temp_user.path(), "test-template-user.yaml", "User Template");
    create_test_template(temp_local.path(), "test-template-local.yaml", "Local Template");
...
fn test_template_source_priority() {
    assert_eq!(TemplateSource::System.priority(), 1, "System should have priority 1");
    assert_eq!(TemplateSource::User.priority(), 2, "User should have priority 2");
    assert_eq!(TemplateSource::Local.priority(), 3, "Local should have priority 3");
...
async fn test_discovery_with_invalid_files() {
    ...
    fs::write(temp_dir.path().join("not-a-template.txt"), "invalid content").unwrap();
...
async fn test_template_listing() {
    ...
    assert!(templates.is_empty() || !templates.is_empty(), "Should return valid vec");
}
...
async fn test_get_template_by_id() {
    let temp_dir = TempDir::new().unwrap();
    create_test_template(temp_dir.path(), "specific-template.yaml", "Specific Template");
    ...
    assert!(result.is_none(), "Should return None for non-existent template");
}
